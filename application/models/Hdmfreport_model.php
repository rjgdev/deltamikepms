...
class Hdmfreport_model extends CI_Model
{
  	...
  	function get_details($month, $year)
	{
		...
		$sum = $this->db->query("
	    	SELECT *,
	    	SUM(hdmf_ee) as totalee,
	    	SUM(hdmf_er) as totaler,
	    	SUM(hdmf_ee + hdmf_er) as totalhdmf
	    	FROM dm_payrolldetails as pd
	    	LEFT JOIN dm_employee as emp
	    	ON pd.employeeID = emp.employeeID
	    	LEFT JOIN dm_payroll as pr
		...
		$date = $this->db->query("
	    	...
	    	LEFT JOIN dm_payroll as pr
	    	ON pd.payrollID = pr.payrollID
	    	WHERE MONTH(pr.dateto) = $month AND YEAR(pr.dateto) = $year AND payperiod = 2
	    	GROUP BY pd.employeeID"
		);
