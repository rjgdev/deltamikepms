...
class Approval_model extends CI_Model
{
    ...
    {
      $employeeType = ($type=="0") ? "" : " AND approval.employeetypeID=".$type;

    	$query = $this->db->query('SELECT * FROM approval 
    							   INNER JOIN employeetype ON employeetype.employeetypeID=approval.employeetypeID 
    							   WHERE approval.moduleID='.$id.$employeeType.' order by approvalDescription');
      return $query->result();
    }
    ...
    function get_approver($id)
    {
      $query = $this->db->query('SELECT * FROM approvaldet 
      						   ...
      						   WHERE approvaldet.approvalID='.$id.' order by  approvalLevel');
      return $query->result();
    }
