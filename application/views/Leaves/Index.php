					...
					<table class="table table-striped custom-table mb-0 datatable">
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-right">
									<button type="button" id="<?php echo $record->employeeleaveID; ?>" class="btn btn-info btn-sm edit_leave" data-toggle="modal" data-target="#edit_leave" data-id="<?php echo $record->employeeleaveID; ?>" data-leavetypeid="<?php echo $record->leavetypeID; ?>" data-employeeid="<?php echo $record->employeeID; ?>"  data-leavetypename="<?php echo $record->leavetypename; ?>" data-leavefrom="<?php echo $record->leavefrom; ?>" data-leaveto="<?php echo $record->leaveto; ?>" data-numberofdays="<?php echo $record->numberofdays; ?>" data-remainingleave="<?php echo $record->remainingleave; ?>" data-reason="<?php echo $record->reason; ?>" data-tog="tooltip"data-placement="top" title="Edit"> <i class="fa fa-pencil"></i> 
								</td>     
							</tr>
							<?php endforeach; ?>
						</tbody>
...
<script  type="text/javascript">  
  $(document).ready(function() {
	...
	$('#save').unbind('click').bind('click', function(){
		...
		if($(IDArray[5]).val()<= ($(IDArray[4]).val() - 1)){	
		...
		if(firstRequired==""){
    		$.ajax({
                ...
                method : "POST",
                data : {leaveID: ValueArray[0], 		employeeID: ValueArray[1],
                		startdate: ValueArray[2], 		enddate: ValueArray[3],
                		numberofdays: ValueArray[4], 	remainingleave: ValueArray[5], reason: ValueArray[6]},
                async : true,
                dataType : 'json',
		...
		if(originalvalue!=newvalue){
			...
			if(lessLeave > remainingLeaves){
				document.getElementById(ErrorIDArray[4]).innerHTML = "Insufficient credit " + $(IDArray[5]).attr("description") +".";
				$(IDArray[4]).addClass('is-invalid');
                event.preventDefault();
				return false;
				}else{
			...
			// add creadit leave
				...
				var lessLeave =  parseInt(remainingLeaves,10) + parseInt(minusdays,10);
		...
		if(firstRequired==""){
    		$.ajax({
                ...
                method : "POST",
                data : {id:id,
                		leaveID: ValueArray[0], 		employeeID: ValueArray[1],
                		startdate: ValueArray[2], 		enddate: ValueArray[3],
                		numberofdays: ValueArray[4], 	remainingleave: ValueArray[5], reason: ValueArray[6], lessLeave: lessLeave},
                async : true,
                dataType : 'json',
                success: function(data){
                var result = data.split('|');
